...
# Intro
When someone tells you that the probability of something happening is $p$ what do they actually mean? 

(Let's assume that person is a mathematician or statistician. Otherwise, I'm not sure what I'm about to write is an accurate answer) 

Here's what they mean: 

* They are observing some experiment (it could be rolling a die, a soccer match, spread of an epidemic, change in crude oil prices) whose outcome is (usually) not known in advance. 
* "something" refers to a set of possible outcomes ("even", "Home team wins by more than one goal difference", "$70\%$ of the population of NYC will be infected by tomorrow", "the price of a barrel will not exceed $\\$75 $ a week from now"), which they can determine whether occurred or not after their observation is complete. Such sets are known as "events". 
* They have a mathematical model that assigns a numerical weight to each event, the weight being a number between $0$ and $1$ ($0\%$ to $100\%$), that can be interpreted as a qualitative measurement of "likelihood" or "chance" of the event. You can think of it as a (careful form of) prediction. That's it. It is important to understand that the probability assigned is model-based. Did you ever notice that Weather.com, NOAA and Weather Underground very often give different "chances of rain" for the same day? If not, try now. 

In this chapter we will explore the mathematical framework for what we just described. 

This framework consists of three components we will now try to briefly explain. 
...
2. **[The sigma-algebra (or $\sigma$-algera)](#Events and sigma-algebras)** (it's just a name, don't dwell on it yet). This represents information we, the observer, have on the outcome at the conclusion of our observation. In some cases, we the observer, may not have complete information on the outcome. An **event** is a statement we will be able to determine as true or false at the conclusion of our observation, and the sigma-algebra is the collection of all events. We will make these two concepts mathematically precise in the relevant section. To understand the concept, let's think of the following case. Two dice are rolled. The sample space therefore $\Omega= \\{1,\dots,6\\}^2 = \\{(i,j):i,j \in \\{1,\dots,6\\}\\}$. We, as the observers, may have 

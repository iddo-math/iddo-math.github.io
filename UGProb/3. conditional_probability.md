...
We're using the uniform probability measure, so the probability of Baylor (or UConn, or any other team) winning is $1/4$. After the semifinal UConn and Notre Dame were eliminated. What is then the probability of Baylor winning the title? $1/2$. What is the probability of UConn winning the title? $0$.  What just happened? We learned something about the outcome after the semifinal. This lead to an adjusted, probability measure, known in our line of work as **conditional probability**, derived from the original probability by using what we learned about the outcome. 
...
\begin{xmpl}\tlabel{2children} It starts normal. A family has two children.
...
1. This one is a little tricky. Not because of the math, but because the question is ambiguous: some may argue that the additional information on day of birth is completely irrelevant, so the answer is identical to b., while some may argue that this information means that the sample space is different and includes the day on which each child is born. With this latter interpretation the sample space is sequences of length two of symbols of the form $x_i$, where $x$ represents the gender, being equal to $b$ or $g$, while $i$ represents the day of the week, $1$ for Sunday, $2$ for Monday, ..., and $7$ for Saturday. Thus, by the product rule, we have $14=2\times 7$ symbols, and we have exactly $14^2$ sequences of length $2$ obtained from these symbols. Time for calculations.
    ...
    - Number of sequences with youngest being a boy is $14\times7$. Among those, exactly $13\times6$ do not have a boy born on a Sunday. Therefore, number of elements in $A\cap B$ is $14\times7-13\times6 = 13+ 7= 20$. 
...
\begin{equation}\label{total_prob}
...
$$
\begin{align}
P(A) &= \sum\limits_{j=1}^8 P(A|B_j) P(B_j)\\
& = 0.016 \times 0.02+0.016 \times 0.02+0.024\times.11\\
&+0.027\times0.24+0.036\times0.11+0.044\times.16+0.052\times.26+0.074\times0.08\\
&=0.04.
\end{align}
$$
\end{xmpl}
...
Similarly, 
\begin{empheq}
\tlabel{bayes1}  P(B\|A) = \frac{P(B \cap A)}{P(A)} = \frac{P(A\|B)P(B)}{P(A)}.
\end{empheq} 
...
\begin{prob}
\tlabel{3children}
Repeat the calculation in \ref{2children} now assuming that the family has three children. No need to do the third part. 
\end{prob}
...
\begin{prob}[Total probability for conditional probabilities]\tlabel{total_condition}
